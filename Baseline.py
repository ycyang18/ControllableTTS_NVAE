...
from VAE_models.vanillaVAE import VAE_Model

class PortaSpeechInterface(torch.nn.Module):

    def __init__(self,
                 device=torch.device('cuda' if torch.cuda.is_available() else 'cpu'),
                 tts_model_path=os.path.join(MODELS_DIR,"PortaSpeech_Meta.pt"),
        ...
        if vocoder_model_path is None:
            vocoder_model_path = os.path.join(MODELS_DIR, "Avocodo.pt")
        ...
        #   load weights               #
        ################################
        checkpoint = torch.load(tts_model_path, map_location=self.device)
        ...
        check_dict = torch.load(os.path.join(MODELS_DIR, "Embedding", "embedding_function.pt"), map_location=self.device) #cpu
        self.style_embedding_function.load_state_dict(check_dict["style_emb_func"])

        #self.style_embedding_function = VAE_Model()
        #check_dict = torch.load(os.path.join(MODELS_DIR, "Checkpoint", "vae_v1.pt"), map_location=self.device) #cpu
        #self.style_embedding_function.load_state_dict(check_dict["model"])

        #self.style_embedding_function.to(self.device)

        #random_latent_vectors = torch.randn(1, 16).to(device)
        #desired_min_variance = 0.1
        #desired_max_variance = 1.9
        ...
        #with torch.no_grad():
        #    self.reconstructed_embeddings = self.style_embedding_function.decoder(random_latent_vectors)
        ################################
        ...
        #self.default_utterance_embedding = self.reconstructed_embeddings.squeeze()
        #print(' * default embedding:', self.default_utterance_embedding, self.default_utterance_embedding.shape)
    ...
    def set_utterance_embedding(self, path_to_reference_audio="", embedding=None):
        ...
        return self.default_utterance_embedding

    def set_utterance_embedding_batch(self, audio_paths):
        batch_embeddings = []
        for path_to_reference_audio in audio_paths:
            assert os.path.exists(path_to_reference_audio)
            wave, sr = soundfile.read(path_to_reference_audio)
            ...
            spec = self.audio_preprocessor.audio_to_mel_spec_tensor(wave).transpose(0, 1)
            spec_len = torch.LongTensor([len(spec)])
            embedding = self.style_embedding_function(spec.unsqueeze(0).to(self.device), spec_len.unsqueeze(0).to(self.device)).squeeze()
            batch_embeddings.append(embedding)

        batch_embeddings = torch.stack(batch_embeddings)  # Convert list of embeddings to a batch tensor
...
vae_model = VAE_Model()
check_dict = torch.load(os.path.join(MODELS_DIR, "Checkpoint", "vae_v1.pt"), map_location=device) #cpu
vae_model.load_state_dict(check_dict["model"])
vae_model.to(device)

n_sample = 1
n_dim = 16
random_latent_vectors = torch.randn(n_sample, n_dim).to(device)
desired_min_variance = 1
desired_max_variance = 5
scaling_factor = (desired_max_variance - desired_min_variance) / 2.0
random_latent_vectors = random_latent_vectors * scaling_factor + (desired_max_variance + desired_min_variance) / 2.0
with torch.no_grad():
    reconstructed_embeddings = vae_model.decoder(random_latent_vectors)

text = "Hello, this is a test sentence."
utterance_embedding = reconstructed_embeddings.squeeze()

generated_audio = tts_interface(text, utterance_embedding)
generated_audio = generated_audio.cpu().numpy()
soundfile.write("output_vae[1-5].wav", generated_audio, 16000)