...
class InferenceFastSpeech2(torch.nn.Module):
    ...
    def __init__(self,
        ...
        #change
        checkpoint = torch.load(os.path.join(MODELS_DIR, f"FastSpeech2_Meta.pt"), map_location=self.device)
        ...
        #change
        check_dict = torch.load(os.path.join(MODELS_DIR, "Embedding", "embedding_function_fast.pt"), map_location=self.device)
        ...
        ################################
        #self.mel2wav = torch.jit.trace(HiFiGANGenerator(path_to_weights=os.path.join(MODELS_DIR, "Avocodo", "best.pt")),torch.rand((80, 50))).to(torch.device(device))
        #change
        self.mel2wav = torch.jit.trace(HiFiGANGenerator(path_to_weights=os.path.join(MODELS_DIR, "Avocodo_fast.pt")),torch.rand((80, 50))).to(torch.device(device))
    ...
    def set_utterance_embedding(self, path_to_reference_audio="", embedding=None):
        ...
        return self.default_utterance_embedding #change
    ...
    def forward(self,
                text,
                utterance_embedding=None, #change
                view=False,
        ...
        with torch.inference_mode():
            ...
            mel, durations, pitch, energy = self.phone2mel(phones,
                                                           ...
                                                           utterance_embedding= utterance_embedding, #self.default_utterance_embedding,
                                                           durations=durations,
                                                           pitch=pitch,
                                                           energy=energy,
