'''
changes marked as ##porta_change
'''
import itertools
import os
...
class PortaSpeechInterface(torch.nn.Module):
    ...
    def __init__(self,
                 ...
                 tts_model_path=os.path.join(MODELS_DIR, f"PortaSpeech_Meta.pt"), ##porta_change
        ...
        if vocoder_model_path is None:
            vocoder_model_path = os.path.join(MODELS_DIR, "Avocodo.pt") ##porta_change
        ...
        #   load weights               #
        ################################
        checkpoint = torch.load(tts_model_path, map_location=self.device) #cpu
        ...
        check_dict = torch.load(os.path.join(MODELS_DIR, "Embedding", "embedding_function_porta.pt"), map_location=self.device) #cpu
    ...
    def forward(self,
                text,
                utterance_embedding=None, ##porta_change
                view=False,
        ...
        with torch.inference_mode():
            ...
            mel, durations, pitch, energy = self.phone2mel(phones,
                                                           ...
                                                           utterance_embedding=utterance_embedding, ##porta_change
                                                           durations=durations,
                                                           pitch=pitch,
                                                           energy=energy,
