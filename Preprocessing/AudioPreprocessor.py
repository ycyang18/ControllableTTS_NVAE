...
class AudioPreprocessor:
    ...
    def __init__(self, input_sr, output_sr=None, melspec_buckets=80, hop_length=256, n_fft=1024, cut_silence=False, device="cpu", fmax_for_spec=8000):
        ...
        if cut_silence:
            ...
            #print(' * silero_model:', self.silero_model)
        else:
    ...
    def audio_to_mel_spec_tensor(self, audio, normalize=True, explicit_sampling_rate=None):
        ...
        if explicit_sampling_rate is None:
            if normalize:
                ...
                ## Check for non-finite/nan values
                if not np.isfinite(audio).all():
                    print('** audio buffer in infinite:', audio)
                    audio[np.isnan(audio)] = 0.0
                if torch.isnan(audio).any():
                    print('** audio Nan:', audio)
                    audio[np.isnan(audio)] = 0.0
...
if __name__ == '__main__':
    ...
    wav, sr = soundfile.read("/mount/resources/speech/corpora/ADEPT/wav_44khz/emotion/joy/ad00_0000.wav")
    ap = AudioPreprocessor(input_sr=sr, output_sr=16000, cut_silence=False, device='cuda')
